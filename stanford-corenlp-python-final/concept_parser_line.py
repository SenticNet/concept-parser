...
class concept_parser_v2:												# Class containing all word pair combination functions
	...
	def dobj(self,line):
		...
		if len(final_dobj) > 1:
			...
			final_concept.append(final_dobj[0])
	...
	def acomp(self,line):
		...
		if len(final_acomp) > 1:
			...
			final_concept.append(final_acomp[0])
	...
	def amod(self,line):
		...
		if len(final_amod) > 1:
			...
			final_concept.append(final_amod[0])
	...
	def nn(self,line):
		...
		if len(posit_sum) > 1:
			...
			final_concept.append(final_nn[0])
