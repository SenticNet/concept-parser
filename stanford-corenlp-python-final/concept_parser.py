...
def main():
	...
	input_file = open('NE-tweets.txt','r')									# Input file containing sentences to parse
	output_file = open('Input2.txt','a+')
	count0 = 1
	for senten in input_file.readlines():
		senten = senten.strip()  # the while loop will leave a trailing space, 
		...
		if not position:
			...
			for i in position:
				and_list = concept.conjugator(words,postags,int(i))				# conjugator Call
		...
		for i in range(0,len(final_concepts)):
			if len(final_concepts[i]) >= 2:
				for j in range(0,len(final_concepts[i])):
					output_file.write(str(final_concepts[i][j]))								# Final Concepts of current sentence
					output_file.write(',')
			else:
					output_file.write(str(final_concepts[i]))								# Final Concepts of current sentence
					output_file.write(',')
			output_file.write('\n')
